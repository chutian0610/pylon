...
fn agg(name: &str, func: &str, arg: Option<&str>, input: Option<DataType>, out: DataType) -> AggSpec {
    AggSpec {
        func: func.into(),
...
async fn count_star_group_by_one_column() {
    ...
    let mut op = HashAggregateOp::new(
        vec!["region".into()],
        vec![agg("cnt", "count", None, None, DataType::Int64)],
        output_schema,
    );
...
async fn min_max_on_int_and_string() {
    ...
    let mut op = HashAggregateOp::new(
        vec!["region".into()],
        vec![
            agg("lo_id", "min", Some("id"), Some(DataType::Int64), DataType::Int64),
            agg("hi_id", "max", Some("id"), Some(DataType::Int64), DataType::Int64),
            agg("lo_r", "min", Some("region"), Some(DataType::Utf8), DataType::Utf8),
            agg("hi_r", "max", Some("region"), Some(DataType::Utf8), DataType::Utf8),
        ],
        output_schema,
    );
    op.add_input(make_input_batch()).await.unwrap();
...
async fn multiple_aggregates_in_one_op() {
    ...
    let mut op = HashAggregateOp::new(
        vec!["region".into()],
        vec![
            agg("cnt", "count", None, None, DataType::Int64),
            agg("total", "sum", Some("amount"), Some(DataType::Float64), DataType::Float64),
            agg("lo", "min", Some("id"), Some(DataType::Int64), DataType::Int64),
            agg("hi", "max", Some("id"), Some(DataType::Int64), DataType::Int64),
        ],
        output_schema,
    );
    op.add_input(make_input_batch()).await.unwrap();
