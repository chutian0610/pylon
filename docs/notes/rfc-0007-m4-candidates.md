...
| **C5.6** | P2 follow-ups: S3 sink multipart/chunked upload (removes full-in-memory buffering + 5 GB PUT cap); per-query QSM map cleanup (`attempts`/`stalled`/`acked`/`expected`); mutex-poisoning policy (`into_inner`); S3 orphan-object GC (bucket lifecycle rule or coord sweep). | Production hardening before S7/S8 chaos + 1B-row E2E. | ~4 dev days |
