...
| **M4.S8** | 1B-row mid-flight-worker-kill E2E — the headline M4 sign-off test. Pre-reqs now satisfied (FTE source DONE: input replay gives exact mid-task-kill correctness; S7 testbed provides the harness — `fte_kill_e2e.sh` at 100k scale is the template; 1B rows additionally needs the `object_store` S3 spill root so worker-local replay logs survive cross-host, or a single-host run). | M4 sign-off. | ~3 dev days | FTE source (DONE) + S7 (DONE) |
