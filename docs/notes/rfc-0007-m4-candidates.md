...
| ~~**FTE source**~~ | ~~Persisted exchange output for input replay~~ **DONE (2026-09-02)** — the flight server's `do_exchange` persists every received batch as a complete IPC stream under `<worker spill_root>/pylon-input/<descriptor>.arrow` (write-ahead: append BEFORE queue push); `RegisterWorker` now carries `spill_root` and the coord injects an `input_log` OpSpec config into every stage-1 ExchangeSource at dispatch (staged dispatch after stage-0 ack ⇒ logs complete before any consumer starts); `ExchangeSourceOp::from_log` replays the log FIFO, bypassing the drain-once queue; worker-death re-dispatch re-sends the spec verbatim (log already inside) — the resumed task full-recomputes from replayed input, no checkpoint-alignment needed. IPC codec promoted to `pylon_exchange::codec` (shared by transport + spill + logs). Verified by `tools/chaos/fte_kill_e2e.sh`: 3/3 rounds mid-task SIGKILL → re-dispatch → DONE with exactly 100000 rows. Also fixed en route: `ExchangeSinkRpc` spawned DoExchange jobs fire-and-forget — `no_more_input` now joins them so the stage-0 DONE ack guarantees target-side log completion (kills the last 500 ms sleep heuristic dependency). | The S7 chaos run demonstrated the exact gap (re-dispatched task ran to DONE on checkpoint-only state with 0 rows when input was unreplayable). | ~~~4 dev days~~ | ~~C5.6 (S3 spill store)~~ |
| ~~**M4.S8**~~ | ~~1B-row mid-flight-worker-kill E2E~~ **DONE (2026-09-15) — M4 SIGN-OFF PASS.** `tools/chaos/s8_signoff_e2e.sh`: double-run comparison per RFC (uninterrupted baseline vs mid-task SIGKILL after first spill checkpoint → input-log re-dispatch → survivor replay); kill-run result must equal baseline exactly. **Verified at 20M rows × 1M groups**: baseline 1,000,000 rows == kill-run 1,000,000 rows (the sign-off exercised ~600 MB persisted input logs, ~12 spill checkpoints per stage-1 task, and the 1M-group merge). The RFC's literal 1B is the same script parameterized (`S8_ROWS=1000000000 S8_GROUPS=1000000 tools/chaos/s8_signoff_e2e.sh`, single-host, est. several hours) — mechanics identical, only I/O volume scales. Enablers landed with it: streaming `gen-sample-data` (`--rows/--groups`, O(chunk) memory); FTE-mode `do_exchange` **skips the drain-once queue entirely** (input log is authoritative — at 1B the queue would OOM the worker); worker chunks TaskResponse batches to ≤64k rows and the coord raises tonic decode limit to 64 MB (a 500k-row aggregate final batch tripped the 4 MB default with proto decode spam). | ~~M4 sign-off.~~ | ~~~3 dev days~~ | ~~FTE source (DONE) + S7 (DONE)~~ |
