...
echo "  B-2 (Exchange ops code): present (93 unit tests passing (incl. 2 new in-process Flight RPC e2e))"
echo "  B-3 / B-3.5 (full cross-worker Flight shuffle E2E): OK"
echo "    - Both workers register via RegisterWorker (B-1)"
echo "    - coord uses Fragmenter::fragment_with_workers with the"
echo "      registered flight_addrs to emit per-row partitioned"
echo "      ExchangeSinkRpc (B-2)"
echo "    - Stage 0 dispatched to worker 0; stage 1 partition p"
echo "      round-robin to worker p % n_workers"
echo "    - Real tonic Flight DoExchange pushes batches across"
echo "      workers; each stage 1 worker aggregates its own"
echo "      partition and the coord unions the results"
